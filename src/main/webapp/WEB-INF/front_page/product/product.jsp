			...
			<div class="mt ofc">
				<ul class="uls i_150x150 x4_150x150b">
					<c:forEach items="${pagination.list}" var="entry">	
						<li>
							<a href="javascript:void(0)" onclick="window.open('/product/detail.shtml?prodcutId=${entry.id}')" title="${entry.name }"  class="pic"><img src="${entry.img.allUrl}" alt="${entry.name }" /></a>
							<dl>
								<!-- dt 10个文字+... -->
								<dt><a href="javascript:void(0)" onclick="window.open('/product/detail.shtml?prodcutId=${entry.id}')" title="${entry.name }" >${entry.name }</a></dt>
								<!-- dt 25个文字+... -->
