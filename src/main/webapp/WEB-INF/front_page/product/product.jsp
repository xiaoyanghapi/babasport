		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			<ul class="uls filter">
				...
				<li><b>类型：</b><p>
					...
					<a href="javascript:void(0);" title="${type.name }" onclick="window.location.href='/product/display/list.shtml?brandId=${brandId}&brandName=${brandName}&typeId=${type.id}&typeName=${type.name}'">${type.name }</a>
					</c:forEach>
				</p>
				</li>
				</c:if>
			...
			<div class="mt ofc">
				<ul class="uls i_150x150 x4_150x150b">
					<c:forEach items="${pagination.list}" var="entry">	
						<li>
							<a href="javascript:void(0)" onclick="window.open('/product/detail.shtml?productId=${entry.id}')" title="${entry.name }"  class="pic"><img src="${entry.img.allUrl}" alt="${entry.name }" /></a>
							<dl>
								<!-- dt 10个文字+... -->
								<dt><a href="javascript:void(0)" onclick="window.open('/product/detail.shtml?productId=${entry.id}')" title="${entry.name }" >${entry.name }</a></dt>
								<!-- dt 25个文字+... -->
